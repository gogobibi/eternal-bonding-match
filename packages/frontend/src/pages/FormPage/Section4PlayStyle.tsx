...
export default function Section4PlayStyle({ data, onChange }: { data: ProfileInput; onChange: (u: Partial<ProfileInput>) => void }) {
  const items = data.play_styles ?? []

  const add = () => onChange({ play_styles: [...items, { id: crypto.randomUUID(), text: '', emphasized: false }] })
  const remove = (id: string) => onChange({ play_styles: items.filter(i => i.id !== id) })
  const update = (id: string, patch: Partial<PlayStyleItem>) => onChange({ play_styles: items.map(i => i.id === id ? { ...i, ...patch } : i) })
    ...
    <div className="space-y-4">
      ...
      {items.map(item => (
        <div key={item.id} className="flex gap-2 items-center">
          <input type="text" value={item.text} onChange={e => update(item.id, { text: e.target.value })} placeholder="플레이 스타일을 입력해 주세요"
