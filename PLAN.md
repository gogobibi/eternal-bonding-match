...
- [x] 성향 데이터 저장 API (POST /profiles)
- [x] 공유 링크 생성 API (POST /links)
- [x] 링크 기반 프로필 조회 API (GET /links/:linkId)
...
| 섹션 | 항목 |
|------|------|
| 1. 기본 정보 | [나] 닉네임, 서버(5개), 성별, 나이대, 접속 시간대(평일/주말)<br>[상대] 성별, 나이대, 접속 시간대(평일/주말, 무관 옵션) |
| 2. 캐릭터 | 커플링 우선순위(BL/GL/HL, 복수 선택 가능), [나] 종족(20종), [상대] 종족(20종) |
| 3. 컨텐츠 성향 | [나] 주직(21개 직업), 컨텐츠 선택, 커스텀 키워드<br>[상대] 상대 컨텐츠 요구 여부, 직업, 컨텐츠, 커스텀 키워드 |
